...
// ---------------------------------------------------------------------------
// COP1 (opcode == 0x11): absent on PS1 hardware. Decode architectural fields
// so the strict translator can emit Coprocessor Unusable / NOP-like behavior.
// ---------------------------------------------------------------------------
void MipsDecoder::decode_cop1(DecodedInstruction& out) {
    out.format = InstrFormat::COP1;
    switch (out.rs) {
        case 0x00: out.mnemonic = "MFC1"; break;
        case 0x02: out.mnemonic = "CFC1"; break;
        case 0x04: out.mnemonic = "MTC1"; break;
        case 0x06: out.mnemonic = "CTC1"; break;
        case 0x08:
        case 0x0C: out.mnemonic = (out.rt & 1u) ? "BC1T" : "BC1F"; break;
        default:   out.mnemonic = "COP1"; break;
    }
}
...
void MipsDecoder::set_classification_flags(DecodedInstruction& out) {
    ...
    if (out.opcode == 0x11 && (out.rs == 0x08 || out.rs == 0x0C)) {
        out.is_branch = true;
        out.is_delay_slot_user = true;
        return;
    }
...
DecodedInstruction MipsDecoder::decode(uint32_t instr, uint32_t address) {
    ...
    switch (out.opcode) {
        ...
        case 0x11: decode_cop1(out); break;
        ...
        case 0x31: out.mnemonic = "LWC1";  out.format = InstrFormat::I; break;
        case 0x32: out.mnemonic = "LWC2";  out.format = InstrFormat::I; break;
        case 0x39: out.mnemonic = "SWC1";  out.format = InstrFormat::I; break;
    ...
    if (out.opcode == 0x04 || out.opcode == 0x05 ||
        ...
        out.opcode == 0x01 ||
        (out.opcode == 0x11 && (out.rs == 0x08 || out.rs == 0x0C))) {
        // Integer branches plus COP1 BC1F/BC1T.
