...
/* Synchronous exception helpers shared by statically recompiled code and the
 * sanctioned runtime interpreters. COP1/COP3 do not exist on PS1 hardware:
 * Beetle raises Coprocessor Unusable when the matching CU bit is clear; when
 * enabled, their non-branch operations are NOP-like. Keep the exception state
 * update here so every backend publishes identical Cause/EPC/SR/vector state. */
static inline void psx_sync_exception(CPUState* cpu, uint32_t code, uint32_t ce,
                                      uint32_t pc) {
    uint32_t sr = cpu->cop0[12];
    cpu->cop0[12] = (sr & ~0x3Fu) | ((sr << 2) & 0x3Fu);
    cpu->cop0[13] = (cpu->cop0[13] & 0x0000FF00u)
                  | ((code & 0x1Fu) << 2) | ((ce & 3u) << 28);
    cpu->cop0[14] = pc;
    cpu->pc = (sr & 0x00400000u) ? 0xBFC00180u : 0x80000080u;
}

static inline void psx_cop_unusable(CPUState* cpu, uint32_t cop, uint32_t pc) {
    psx_sync_exception(cpu, 11u, cop, pc);
}

static inline void psx_reserved_instruction_exception(CPUState* cpu,
                                                       uint32_t pc) {
    psx_sync_exception(cpu, 10u, 0u, pc);
}

static inline void psx_address_error_exception(CPUState* cpu, uint32_t addr,
                                               uint32_t pc, int is_store) {
    cpu->cop0[8] = addr;
    psx_sync_exception(cpu, is_store ? 5u : 4u, 0u, pc);
}
