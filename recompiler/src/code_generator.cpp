    ...
    for (const Function& func : functions) {
        ...
        if (func.is_data_section) {
            throw std::runtime_error(fmt::format(
                "refusing to emit data range 0x{:08X}..0x{:08X} as function {}; "
                "filter it in discovery and emit a data-range manifest",
                func.start_addr, func.end_addr, func.name));
        }
        ...
        // Unsupported instructions are generator failures, never silent TODOs
        // and never function-level unknown-dispatch fallbacks.
        size_t todo = 0, pos = 0;
        while ((pos = gen_func.body.find("TODO: opcode", pos)) != std::string::npos) {
            todo++;
            pos += 12;
        }
        pos = 0;
        while ((pos = gen_func.body.find("TODO: SPECIAL", pos)) != std::string::npos) {
            todo++;
            pos += 13;
        }
        if (todo != 0) {
            throw std::runtime_error(fmt::format(
                "function {} (0x{:08X}..0x{:08X}) contains {} unsupported "
                "instruction emit(s); fix the translator",
                func.name, func.start_addr, func.end_addr, todo));
        }
...
std::string CodeGenerator::generate_file(
    ...
    // Snapshot the exact post-split inputs used below so the companion range
    // manifest describes every emitted entry, including pre-pass additions.
    emitted_functions_ = functions_mut;
    emitted_cfgs_ = cfgs_mut;
    emitted_snapshot_valid_ = true;
...
std::string CodeGenerator::generate_emitted_ranges_manifest() {
    if (!emitted_snapshot_valid_) {
        throw std::logic_error(
            "generate_emitted_ranges_manifest called before generate_file");
    }
    return generate_ranges_manifest(emitted_functions_, emitted_cfgs_);
}
