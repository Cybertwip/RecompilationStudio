...
FunctionDiscovery::CFInfo FunctionDiscovery::classify_control_flow(uint32_t raw, uint32_t addr) {
    ...
    // COP1 conditional branch (BC1F/BC1T; rs 0x08/0x0C). COP1 is absent
    // on PS1, but with CU1 enabled Beetle still treats this as a delayed branch.
    if (op == 0x11) {
        uint8_t cop_op = (raw >> 21) & 0x1Fu;
    ...
    if (op == 0x00) {
        ...
        switch (funct) {
            ...
            case 0x06: case 0x07: case 0x10: case 0x11:
            case 0x12: case 0x13: case 0x18: case 0x19:
            case 0x1A: case 0x1B: case 0x20: case 0x21:
            case 0x22: case 0x23: case 0x24: case 0x25:
            case 0x26: case 0x27: case 0x2A: case 0x2B:
                break; // valid non-control SPECIAL instruction
            default:
                info.kind = CFKind::ReservedInstruction;
                return info;
        }
    }
    ...
    const std::string&          lineage)
{
    ...
    while (!work.empty()) {
        ...
        switch (cf.kind) {
            ...
            case CFKind::ReservedInstruction:
                result.exit_types.insert("reserved_instruction_exception");
                break;
        }
    }
                ...
                if (is_last_before_next) {
                    switch (cf.kind) {
                        ...
                        case CFKind::RFE:
                            ...
                            break;
                        case CFKind::ReservedInstruction:
                            bb.termination = "reserved_instruction_exception";
                            break;
                        case CFKind::Normal:
                            bb.termination = "fall_through";
                            break;
