...
public:
    ...
    // Seed the latch directly. This port runs standalone BIOS HLE: there are no
    // BIOS bytes and the region is never executed, so latch_bios_prefetch()
    // never runs and the latch would sit at 0 forever. A cart read of the
    // protected BIOS region would then return 0 rather than the post-handoff
    // prefetch, which is the MC-HP-002 failure described above — the frame-list
    // walker that only terminates because the open-bus high byte has bit 7 set.
    // src/mvii/main.cpp seeds it with 0xE3A02004, the word a real GBA leaves
    // latched at the SWI-return exit (BIOS mem[0x190]).
    void set_bios_open_bus(uint32_t word) { bios_prefetch_ = word; }
