...
static void handle_dirty_ram_stats(int id, const char *json)
{
    ...
    extern int dirty_ram_text_image_active(void);
    extern uint64_t dirty_ram_text_native_blocked(void);
    ...
    int n = snprintf(buf, sizeof(buf),
             ...
             "\"text_registered\":%d,\"text_active\":%d,"
             "\"text_native_blocked\":%llu,\"text_diverged_pages\":%u,"
...
static void handle_gpu_state(int id, const char *json)
{
    ...
    GpuWsDebug ws;
    gpu_ws_get_debug(&ws);
    extern uint64_t g_prim_size_rejects;
...
static void handle_frontend_input_state(int id, const char *json)
{
    (void)json;
    extern int frontend_input_state_json(char *out, int cap);
    char state[16384];
    int n = frontend_input_state_json(state, (int)sizeof(state));
    if (n <= 0) { send_err(id, "frontend input state unavailable"); return; }
    send_fmt("{\"id\":%d,\"ok\":true,\"state\":%s}\n", id, state);
}
...
static const CmdEntry s_commands[] = {
    ...
    { "frontend_input_state", handle_frontend_input_state },
