...
static const Form kForms[] = {
    ...
    { I_TYPE(0x23, V1, V0, 0), "lw_v0_0_v1",
      must_contain({"gpr[2]", "gpr[3]", "psx_cyc_load_word"}) },
    ...
    { I_TYPE(0x20, V1, V0, 0), "lb_v0_0_v1",
      must_contain({"gpr[2]", "gpr[3]", "psx_cyc_load_byte", "int8_t"}) },

    // LBU $v0, 0($v1)
    { I_TYPE(0x24, V1, V0, 0), "lbu_v0_0_v1",
      must_contain({"gpr[2]", "gpr[3]", "psx_cyc_load_byte"}) },

    // LH $v0, 0($v1)
    { I_TYPE(0x21, V1, V0, 0), "lh_v0_0_v1",
      must_contain({"gpr[2]", "gpr[3]", "psx_cyc_load_half", "int16_t"}) },

    // LHU $v0, 0($v1)
    { I_TYPE(0x25, V1, V0, 0), "lhu_v0_0_v1",
      must_contain({"gpr[2]", "gpr[3]", "psx_cyc_load_half"}) },
    ...
    // Architecturally reserved SPECIAL funct -> Reserved Instruction exception.
    { 0x00203A41u, "reserved_special_ri_exception",
      [](const TranslateResult& r) -> std::string {
          if (!r.supported) return fmt::format("unsupported: {}", r.fail_reason);
          if (!r.is_terminator) return "reserved instruction not marked terminator";
          if (!r.terminator_kind || std::strcmp(r.terminator_kind, "reserved_instruction") != 0)
              return "reserved instruction wrong terminator kind";
          if (!contains(r.c_code, "psx_reserved_instruction_exception"))
              return "missing Reserved Instruction exception helper";
          return "";
      } },

    // === COP1 absent-hardware semantics (BIOS strtod at BFC02324) ===
    { 0x44801000u, "cop1_mtc1_copu_or_nop",
      must_contain({"cop0[12]", "1u << 29", "psx_cop_unusable", "return"}) },

    { 0x462C0102u, "cop1_mul_d_copu_or_nop",
      must_contain({"cop0[12]", "psx_cop_unusable", "NOP-like"}) },

    { 0x45000003u, "bc1f_hardwired_false_condition",
      [](const TranslateResult& r) -> std::string {
          if (!r.supported) return fmt::format("unsupported: {}", r.fail_reason);
          if (!r.is_terminator) return "BC1F not marked terminator";
          if (!r.terminator_kind || std::strcmp(r.terminator_kind, "branch_bc1f") != 0)
              return "BC1F wrong terminator kind";
          if (!contains(r.pre_delay_code, "psx_cop_unusable"))
              return "BC1F missing pre-delay Coprocessor Unusable check";
          if (!contains(r.pre_delay_code, "= 1u"))
              return "BC1F is not hardwired taken when CU1 is enabled";
          return "";
      } },

    { 0xC422DC60u, "lwc1_copu_or_discard_read",
      must_contain({"psx_cop_unusable", "psx_address_error_exception", "psx_cyc_cop_load_word"}) },

    { 0xE422DC60u, "swc1_copu_or_no_write",
      check_code([](const std::string& c) -> std::string {
          if (!contains(c, "psx_cop_unusable")) return "missing COPU path";
          if (!contains(c, "psx_address_error_exception")) return "missing alignment exception";
          if (contains(c, "write_word")) return "SWC1 incorrectly writes memory";
          return "";
      }) },
