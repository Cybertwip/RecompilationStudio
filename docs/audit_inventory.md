...
|     ~ | `mov.d, mtc1, lwc1, add.d` | `COP1/LWC1` | 0xBFC0233C | Ghidra proves `FUN_bfc02324` is a real BIOS `strtod`-style function called at `0xBFC02598`; PS1 has no COP1, so the correct semantics are COPU when CU1 is clear and NOP-like/discard behavior when set (Beetle oracle). |
...
- **False positives (data-in-code):** dadd, dmult, dsll32 and other R4000+
  forms still appear when whole regions are scanned without skipping inline
  data. COP1 is a separate class: the `0xBFC02324` sequence is real executable
  BIOS code, but the retail PS1 lacks COP1, so it must emit the hardware COPU /
  enabled-NOP behavior rather than host floating-point arithmetic.
