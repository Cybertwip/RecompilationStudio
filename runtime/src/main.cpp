...
#ifndef PSX_EXPECTED_BIOS_NAME
#define PSX_EXPECTED_BIOS_NAME "SCPH1001.BIN"
#endif
#ifndef PSX_EXPECTED_BIOS_CRC32
#define PSX_EXPECTED_BIOS_CRC32 0x37157331u
#endif
...
#endif
#ifndef PSX_APP_SUPPORT_DIR_NAME
#define PSX_APP_SUPPORT_DIR_NAME ""
#endif
...
/* Branding-patcher presentation gate. A marked BIOS is allowed to execute and
 * render every authentic setup frame, but the frontend remains black until the
 * framebuffer actually matches the replacement first-logo TIM. This is a
 * presentation-only policy: no guest timing, GPU command, or BIOS state is
 * skipped. The marker lives in verified zero padding after the rewritten TIM. */
static constexpr size_t kBrandingGateMarkerOffset = 0x5E2D0u;
static constexpr char kBrandingGateMarker[12] = {
    'P', 'S', 'X', 'B', 'R', 'G', 'A', 'T', 'E', '0', '0', '1'
};
static constexpr int kBrandingGateGridW = 12;
static constexpr int kBrandingGateGridH = 9;
static bool g_branding_present_gate_enabled = false;
static bool g_branding_present_gate_open = false;
static bool g_branding_present_gate_black_presented = false;
static std::array<uint16_t, kBrandingGateGridW * kBrandingGateGridH>
    g_branding_present_gate_expected{};
 ...
 * SDL/direct-USB controller (or uses the keyboard) and feeds each PSX pad slot. */
struct PlayerInput {
...
/* Keyboard edge latch: SDL key-down events are held for three guest vblanks so
...
typedef struct FrontendInputTraceEntry {
    ...
    uint16_t mask;
    uint16_t scancode;
    uint8_t  kind;  /* 1=SDL keydown, 2=sampled P1 pad change */
} FrontendInputTraceEntry;
#define FRONTEND_INPUT_TRACE_CAP 64
...
extern "C" int frontend_input_state_json(char *out, int cap) {
    ...
    for (uint64_t i = 0; i < count && n < cap - 256; i++) {
        ...
        const char *kind_name = e->kind == 1 ? "keydown" : "pad";
        const char *key_name = SDL_GetScancodeName((SDL_Scancode)e->scancode);
        n += std::snprintf(out + n, (size_t)(cap - n),
            "%s{\"seq\":%llu,\"frame\":%u,\"kind\":\"%s\","
            "\"scancode\":%u,\"key\":\"%s\",\"mask\":\"0x%04X\","
            "\"pad\":\"0x%04X\"}",
            i ? "," : "", (unsigned long long)e->seq, e->frame, kind_name,
            (unsigned)e->scancode, key_name ? key_name : "", e->mask, e->pad_word);
    }
    n += std::snprintf(out + n, (size_t)(cap - n), "]}");
    return n;
}
 ...
 * and then failed every #version 330 shader. macOS exposes core contexts as
 * 3.2 or 4.1 (not 3.3), so request 4.1 there; GLSL 330 is valid in that context. */
static void configure_gl_window_attributes(void) {
...
// Generated app bundles set PSX_APP_SUPPORT_DIR_NAME, which redirects settings,
// input maps, memory cards, overlay caches, and sidecar files into the user's
// Application Support directory. Normal developer builds keep the historical
// executable-directory behavior. PSX_RUNTIME_DATA_DIR is an explicit override
// used by tests and launch wrappers.
static std::filesystem::path runtime_writable_dir(const char* argv0) {
    namespace fs = std::filesystem;
    std::error_code ec;
    if (const char* override_dir = std::getenv("PSX_RUNTIME_DATA_DIR");
        override_dir && override_dir[0]) {
        fs::path p = fs::absolute(override_dir, ec);
        if (ec) p = fs::path(override_dir);
        fs::create_directories(p, ec);
        return p;
    }
#ifdef __APPLE__
    if (PSX_APP_SUPPORT_DIR_NAME[0]) {
        const char* home = std::getenv("HOME");
        if (home && home[0]) {
            fs::path p = fs::path(home) / "Library" / "Application Support" /
                         PSX_APP_SUPPORT_DIR_NAME;
            fs::create_directories(p, ec);
            if (!ec) return p;
        }
    }
...
static std::filesystem::path sidecar_cfg_path(const char* argv0, const char* filename) {
    return runtime_writable_dir(argv0) / filename;
}
...
static uint16_t bios_le16(const std::vector<uint8_t>& data, size_t offset) {
    return (uint16_t)data[offset] | ((uint16_t)data[offset + 1] << 8);
}

static uint32_t bios_le32(const std::vector<uint8_t>& data, size_t offset) {
    return (uint32_t)data[offset] | ((uint32_t)data[offset + 1] << 8) |
           ((uint32_t)data[offset + 2] << 16) | ((uint32_t)data[offset + 3] << 24);
}

static void configure_branding_present_gate(const std::vector<uint8_t>& data) {
    g_branding_present_gate_enabled = false;
    g_branding_present_gate_open = false;
    g_branding_present_gate_black_presented = false;
    g_branding_present_gate_expected.fill(0);

    if (data.size() != 512u * 1024u ||
        kBrandingGateMarkerOffset + sizeof(kBrandingGateMarker) > data.size() ||
        std::memcmp(data.data() + kBrandingGateMarkerOffset,
                    kBrandingGateMarker, sizeof(kBrandingGateMarker)) != 0) {
        return;
    }

    constexpr size_t tim = 0x5CC30u;
    if (tim + 64u > data.size() || bios_le32(data, tim) != 0x10u ||
        bios_le32(data, tim + 4u) != 0x08u || bios_le32(data, tim + 8u) != 44u) {
        return;
    }
    const size_t image_block = tim + 52u;
    const uint16_t image_words = bios_le16(data, image_block + 8u);
    const uint16_t image_h = bios_le16(data, image_block + 10u);
    const uint32_t image_len = bios_le32(data, image_block);
    const uint32_t image_w = (uint32_t)image_words * 4u;
    if (image_w != 120u || image_h != 90u ||
        image_len != 12u + (uint32_t)image_words * image_h * 2u ||
        image_block + image_len > data.size()) {
        return;
    }

    std::array<uint16_t, 16> palette{};
    for (size_t i = 0; i < palette.size(); ++i)
        palette[i] = bios_le16(data, tim + 20u + i * 2u);
    const size_t pixels = image_block + 12u;
    for (int gy = 0; gy < kBrandingGateGridH; ++gy) {
        const uint32_t sy = (uint32_t)((gy * 2 + 1) * (int)image_h) /
                            (uint32_t)(kBrandingGateGridH * 2);
        for (int gx = 0; gx < kBrandingGateGridW; ++gx) {
            const uint32_t sx = (uint32_t)((gx * 2 + 1) * (int)image_w) /
                                (uint32_t)(kBrandingGateGridW * 2);
            const uint8_t packed = data[pixels + sy * (image_w / 2u) + sx / 2u];
            const uint8_t index = (sx & 1u) ? (packed >> 4) : (packed & 0x0Fu);
            g_branding_present_gate_expected[(size_t)gy * kBrandingGateGridW + gx] =
                (uint16_t)(palette[index] & 0x7FFFu);
        }
    }
    g_branding_present_gate_enabled = true;
}
...
static bool validate_bios_for_launch(const std::filesystem::path& path) {
    ...
    if (size != 512 * 1024) {
        launcher_warning("BIOS Warning",
            "The selected BIOS is not 512 KiB. Please select " PSX_EXPECTED_BIOS_NAME ".");
        return false;
    }
    std::vector<uint8_t> data((size_t)size);
    if (!read_at(f, 0, data.data(), data.size())) return false;
    configure_branding_present_gate(data);
    const uint32_t crc = crc32_compute(data.data(), data.size());
    if (crc != PSX_EXPECTED_BIOS_CRC32) {
        char buf[256];
        std::snprintf(buf, sizeof(buf),
            "The selected BIOS CRC32 is %08X, but this build was validated with %s CRC32 %08X.\n\n"
            "The runtime will try it anyway, but boot may fail.",
            crc, PSX_EXPECTED_BIOS_NAME, (unsigned)PSX_EXPECTED_BIOS_CRC32);
...
// Fallback memcard directory used when no game config overrides it. Signed
// app bundles redirect this to Application Support.
static std::filesystem::path default_memcard_dir(const char* argv0) {
    return runtime_writable_dir(argv0);
}
...
static bool sdl_source_pressed(SDL_GameController* handle,
    ...
    case ControllerSource::Kind::Button:
        return SDL_GameControllerGetButton(
            handle, (SDL_GameControllerButton)source.id) != 0;
    case ControllerSource::Kind::AxisPositive:
        return SDL_GameControllerGetAxis(
            handle, (SDL_GameControllerAxis)source.id) > controller_deadzone;
...
#if defined(PSX_HAVE_GIP_GAMEPAD)
static bool gip_snapshot(PsxGipGamepad* gamepad, PsxGipGamepadState* state) {
    if (!state) return false;
    *state = PsxGipGamepadState{};
    if (!gamepad) return false;
    return psx_gip_gamepad_get_state(gamepad, state) != 0;
}

static int gip_axis_value(const PsxGipGamepadState& state,
                          SDL_GameControllerAxis axis) {
    switch (axis) {
    case SDL_CONTROLLER_AXIS_LEFTX:        return state.left_x;
    case SDL_CONTROLLER_AXIS_LEFTY:        return state.left_y;
    case SDL_CONTROLLER_AXIS_RIGHTX:       return state.right_x;
    case SDL_CONTROLLER_AXIS_RIGHTY:       return state.right_y;
    case SDL_CONTROLLER_AXIS_TRIGGERLEFT:
        return (int)((uint32_t)std::min<uint16_t>(state.left_trigger, 1023u) *
                     32767u / 1023u);
    case SDL_CONTROLLER_AXIS_TRIGGERRIGHT:
        return (int)((uint32_t)std::min<uint16_t>(state.right_trigger, 1023u) *
                     32767u / 1023u);
    default:                               return 0;
    }
}

static bool gip_button_pressed(const PsxGipGamepadState& state,
                               SDL_GameControllerButton button) {
    uint16_t mask = 0;
    switch (button) {
    case SDL_CONTROLLER_BUTTON_A:             mask = PSX_GIP_BUTTON_A; break;
    case SDL_CONTROLLER_BUTTON_B:             mask = PSX_GIP_BUTTON_B; break;
    case SDL_CONTROLLER_BUTTON_X:             mask = PSX_GIP_BUTTON_X; break;
    case SDL_CONTROLLER_BUTTON_Y:             mask = PSX_GIP_BUTTON_Y; break;
    case SDL_CONTROLLER_BUTTON_BACK:          mask = PSX_GIP_BUTTON_VIEW; break;
    case SDL_CONTROLLER_BUTTON_START:         mask = PSX_GIP_BUTTON_MENU; break;
    case SDL_CONTROLLER_BUTTON_LEFTSTICK:     mask = PSX_GIP_BUTTON_LEFT_STICK; break;
    case SDL_CONTROLLER_BUTTON_RIGHTSTICK:    mask = PSX_GIP_BUTTON_RIGHT_STICK; break;
    case SDL_CONTROLLER_BUTTON_LEFTSHOULDER:  mask = PSX_GIP_BUTTON_LEFT_BUMPER; break;
    case SDL_CONTROLLER_BUTTON_RIGHTSHOULDER: mask = PSX_GIP_BUTTON_RIGHT_BUMPER; break;
    case SDL_CONTROLLER_BUTTON_DPAD_UP:       mask = PSX_GIP_BUTTON_DPAD_UP; break;
    case SDL_CONTROLLER_BUTTON_DPAD_DOWN:     mask = PSX_GIP_BUTTON_DPAD_DOWN; break;
    case SDL_CONTROLLER_BUTTON_DPAD_LEFT:     mask = PSX_GIP_BUTTON_DPAD_LEFT; break;
    case SDL_CONTROLLER_BUTTON_DPAD_RIGHT:    mask = PSX_GIP_BUTTON_DPAD_RIGHT; break;
    case SDL_CONTROLLER_BUTTON_GUIDE:         return state.guide != 0;
    default:                                  return false;
    }
    return (state.buttons & mask) != 0;
}

static bool gip_source_pressed(const PsxGipGamepadState& state,
                               const ControllerSource& source) {
    switch (source.kind) {
    case ControllerSource::Kind::Button:
        return gip_button_pressed(state,
            (SDL_GameControllerButton)source.id);
    case ControllerSource::Kind::AxisPositive:
        return gip_axis_value(state,
            (SDL_GameControllerAxis)source.id) > controller_deadzone;
    case ControllerSource::Kind::AxisNegative:
        return gip_axis_value(state,
            (SDL_GameControllerAxis)source.id) < -controller_deadzone;
    case ControllerSource::Kind::None:
...
static uint16_t sdl_controller_pad_buttons(SDL_GameController* handle) {
    uint16_t buttons = 0xFFFF;  /* all released */
    if (!handle) return buttons;
    for (const auto& entry : controller_map) {
        for (const auto& source : entry.sources) {
            if (sdl_source_pressed(handle, source)) {
...
#if defined(PSX_HAVE_GIP_GAMEPAD)
static uint16_t gip_controller_pad_buttons(PsxGipGamepad* gamepad) {
    uint16_t buttons = 0xFFFF;
    PsxGipGamepadState state{};
    gip_snapshot(gamepad, &state);
    for (const auto& entry : controller_map) {
        for (const auto& source : entry.sources) {
            if (gip_source_pressed(state, source)) {
                buttons &= (uint16_t)~entry.bit;
                break;
            }
        }
...
static uint16_t physical_pad_buttons(const PlayerInput& p) {
    if (p.handle) return sdl_controller_pad_buttons(p.handle);
#if defined(PSX_HAVE_GIP_GAMEPAD)
    if (p.gip) return gip_controller_pad_buttons(p.gip);
#endif
    return 0xFFFF;
}
...
static void pad_sticks_for(const PlayerInput& p, int player, uint8_t out[4], bool fold_dpad) {
    ...
    if (p.gip) {
        PsxGipGamepadState state{};
        gip_snapshot(p.gip, &state);
        axes_to_pad_pair(state.left_x, state.left_y, &out[0], &out[1]);
        axes_to_pad_pair(state.right_x, state.right_y, &out[2], &out[3]);
        if (fold_dpad) {
            if (state.buttons & PSX_GIP_BUTTON_DPAD_LEFT)  out[0] = 0x00;
            if (state.buttons & PSX_GIP_BUTTON_DPAD_RIGHT) out[0] = 0xFF;
            if (state.buttons & PSX_GIP_BUTTON_DPAD_UP)    out[1] = 0x00;
            if (state.buttons & PSX_GIP_BUTTON_DPAD_DOWN)  out[1] = 0xFF;
        }
    }
#endif
}
...
static bool hybrid_stick_active(const PlayerInput& p) {
    ...
    if (p.gip) {
        PsxGipGamepadState state{};
        gip_snapshot(p.gip, &state);
        return state.left_x > dz || state.left_x < -dz ||
               state.left_y > dz || state.left_y < -dz;
    }
#endif
    return false;
}
...
static bool hybrid_dpad_active(const PlayerInput& p, int player, bool kb_always) {
    ...
    if (p.gip) {
        PsxGipGamepadState state{};
        gip_snapshot(p.gip, &state);
        if (state.buttons & (PSX_GIP_BUTTON_DPAD_LEFT |
                             PSX_GIP_BUTTON_DPAD_RIGHT |
                             PSX_GIP_BUTTON_DPAD_UP |
                             PSX_GIP_BUTTON_DPAD_DOWN)) {
            return true;
        }
    }
#endif
...
static uint16_t dev_all_controllers_buttons() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (h) btn &= sdl_controller_pad_buttons(h);
    }
#if defined(PSX_HAVE_GIP_GAMEPAD)
    for (const PlayerInput& player : g_players) {
        if (player.gip) btn &= gip_controller_pad_buttons(player.gip);
    }
#endif
    return btn;
}
...
#if defined(PSX_HAVE_GIP_GAMEPAD)
    for (const PlayerInput& player : g_players) {
        if (!player.gip) continue;
        PsxGipGamepadState state{};
        gip_snapshot(player.gip, &state);
        uint8_t lx, ly, rx, ry;
        axes_to_pad_pair(state.left_x, state.left_y, &lx, &ly);
        axes_to_pad_pair(state.right_x, state.right_y, &rx, &ry);
        if (lx != 0x80 || ly != 0x80) { st[0] = lx; st[1] = ly; }
        if (rx != 0x80 || ry != 0x80) { st[2] = rx; st[3] = ry; }
    }
#endif
...
static bool branding_present_gate_color_close(uint16_t actual, uint16_t expected) {
    actual &= 0x7FFFu;
    expected &= 0x7FFFu;
    const int ar = actual & 0x1F;
    const int ag = (actual >> 5) & 0x1F;
    const int ab = (actual >> 10) & 0x1F;
    const int er = expected & 0x1F;
    const int eg = (expected >> 5) & 0x1F;
    const int eb = (expected >> 10) & 0x1F;
    return std::abs(ar - er) <= 2 && std::abs(ag - eg) <= 2 &&
           std::abs(ab - eb) <= 2;
}

static bool branding_present_gate_frame_ready(const GpuDisplayInfo& di) {
    if (!g_branding_present_gate_enabled || g_branding_present_gate_open)
        return true;
    /* Direct-to-game boot intentionally never renders the branded BIOS frame.
     * Release the frontend as soon as the compiled game entry has fired. */
    if (fntrace_is_game_started())
        return true;
    if (di.disabled || di.depth24 || di.width < 120u || di.height < 90u)
        return false;

    /* Direct GPU backends keep the authoritative boot frame outside the CPU
     * VRAM mirror. Sync only during the short branded-boot gate window. */
    if (g_gl_active) gl_renderer_sync_cpu();
    if (g_vk_active) vk_renderer_sync_cpu();

    const uint16_t* vram = gpu_get_vram();
    int matched = 0;
    const int total = kBrandingGateGridW * kBrandingGateGridH;
    for (int gy = 0; gy < kBrandingGateGridH; ++gy) {
        const int y0 = ((gy * 2 + 1) * (int)di.height) /
                       (kBrandingGateGridH * 2);
        for (int gx = 0; gx < kBrandingGateGridW; ++gx) {
            const int x0 = ((gx * 2 + 1) * (int)di.width) /
                           (kBrandingGateGridW * 2);
            const uint16_t expected =
                g_branding_present_gate_expected[(size_t)gy * kBrandingGateGridW + gx];
            bool sample_match = false;
            /* Permit a two-pixel raster-rounding difference at scaled edges. */
            for (int dy = -2; dy <= 2 && !sample_match; ++dy) {
                const int y = std::clamp(y0 + dy, 0, (int)di.height - 1);
                for (int dx = -2; dx <= 2; ++dx) {
                    const int x = std::clamp(x0 + dx, 0, (int)di.width - 1);
                    const uint16_t actual =
                        vram[((di.display_y + (uint32_t)y) & 511u) * 1024u +
                             ((di.display_x + (uint32_t)x) & 1023u)];
                    if (branding_present_gate_color_close(actual, expected)) {
                        sample_match = true;
                        break;
                    }
                }
            }
            if (sample_match) matched++;
        }
    }
    return matched * 100 >= total * 70;
}

static void branding_present_gate_present_black_once(void) {
    if (!g_branding_present_gate_enabled || g_branding_present_gate_open ||
        g_branding_present_gate_black_presented || g_headless)
        return;
...
#endif
    g_branding_present_gate_black_presented = true;
}
...
static void sdl_vblank_present(void) {
    ...
    if (!g_headless) {
        for (int s = 0; s < 2; s++) {
            if (g_keyboard_edge_frames[s] > 0 && --g_keyboard_edge_frames[s] == 0)
                g_keyboard_edge_mask[s] = 0;
        }
                                ...
                                game frame that could not present wide) */
    {
        ...
        if (g_branding_present_gate_enabled && !g_branding_present_gate_open) {
            if (branding_present_gate_frame_ready(di)) {
                g_branding_present_gate_open = true;
            } else {
                branding_present_gate_present_black_once();
...
int main(int argc, char** argv) {
    ...
    /* Anchor every relative diagnostic artifact in the writable runtime data
     * directory before installing crash handlers. Signed macOS bundles must
     * never write into Contents/MacOS: doing so mutates sealed resources and
     * invalidates the app signature after the first run. */
    {
        std::error_code runtime_dir_ec;
        const std::filesystem::path runtime_dir = runtime_writable_dir(argv[0]);
        std::filesystem::create_directories(runtime_dir, runtime_dir_ec);
        if (!runtime_dir_ec)
            std::filesystem::current_path(runtime_dir, runtime_dir_ec);
        const std::string data_dir_env = runtime_dir.string();
#ifdef _WIN32
        _putenv_s("PSX_RUNTIME_DATA_DIR", data_dir_env.c_str());
#else
        setenv("PSX_RUNTIME_DATA_DIR", data_dir_env.c_str(), 1);
#endif
    }
    /* Install crash handlers after selecting the writable report directory. */
    ...
    if (game_config_path) {
        try {
            ...
            if (gc.runtime.overlay_cache) {
                std::filesystem::path data_dir = runtime_writable_dir(argv[0]);
                std::string cache_dir = (data_dir / "cache").string();
                overlay_capture_set_out_dir(data_dir.string().c_str());
        ...
        if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER) == 0) {
            ...
            if (lr == psx_launcher::Result::Launch) {
                ...
                PSXRecompV4::save_user_settings(
                    runtime_writable_dir(argv[0]) / "settings.toml", seed);
            }
        }
    }
#endif
  ...
  if (g_headless) {
    ...
    if (g_video_renderer == 1) configure_gl_window_attributes();
  ...
  if (!g_gl_active && !g_vk_active) {
    ...
    /* A branded BIOS starts with the frontend explicitly black. Subsequent
     * pre-logo vblanks are suppressed until their framebuffer matches the
     * replacement TIM decoded by validate_bios_for_launch(). */
    branding_present_gate_present_black_once();
