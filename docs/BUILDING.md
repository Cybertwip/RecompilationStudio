...
| **libusb 1.0** | System on macOS, found via pkg-config and statically linked when the archive is available. | Direct wired Xbox GIP input for PDP/Microsoft/PowerA controllers SDL cannot enumerate |
...
**macOS:**
```sh
brew install sdl2 libusb pkg-config ninja cmake
```
...
| `PSX_MACOS_GIP_GAMEPAD` | ON on macOS | Direct libusb backend for wired Xbox GIP controllers SDL cannot expose |
