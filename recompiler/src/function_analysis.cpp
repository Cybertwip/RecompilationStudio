...
bool FunctionAnalyzer::is_valid_mips_word(uint32_t instr) {
    ...
    case 0x10u: case 0x11u: case 0x12u:
    case 0x20u: case 0x21u: case 0x22u: case 0x23u:
...
static uint32_t move_from_ra_destination(uint32_t instr) {
    ...
    if (rd == 0 || (funct != 0x21u && funct != 0x25u)) return 0;
    if ((rs == 31u && rt == 0u) || (rt == 31u && rs == 0u)) return rd;
    return 0;
}

static bool instruction_writes_gpr(uint32_t instr, uint32_t reg) {
    ...
    if (opcode == 0) {
        ...
        case 0x0Cu: // syscall
        case 0x0Du: // break
        case 0x10u: // mfhi writes rd -- handled below
        case 0x12u: // mflo writes rd -- handled below
        case 0x11u: // mthi
        case 0x13u: // mtlo
        case 0x18u: case 0x19u: case 0x1Au: case 0x1Bu: // mult/div
            break;
        default:
            break;
        }
        if (funct == 0x08u || funct == 0x0Cu || funct == 0x0Du ||
            funct == 0x11u || funct == 0x13u ||
            (funct >= 0x18u && funct <= 0x1Bu)) return false;
        return rd == reg;
    }
    if (opcode == 0x02u || opcode == 0x04u || opcode == 0x05u ||
        opcode == 0x06u || opcode == 0x07u ||
        opcode == 0x28u || opcode == 0x29u || opcode == 0x2Bu ||
        opcode == 0x2Eu || opcode == 0x32u || opcode == 0x3Au) return false;
    if (opcode == 0x03u) return reg == 31u;
    if (opcode == 0x01u) {
        const uint32_t kind = rt;
        return (kind == 0x10u || kind == 0x11u) && reg == 31u;
    }
    if ((opcode >= 0x10u && opcode <= 0x13u) && ((instr >> 21) & 0x1Fu) != 0u)
        return false;
    return rt == reg;
}
...
static uint32_t skip_leading_padding_nops(const PS1Executable& exe,
                                          uint32_t start_addr,
                                          uint32_t limit_addr) {
    // Psy-Q leaf routines can be separated by cache-line/alignment padding
    // larger than 32 bytes. Evil Zone has 60 bytes of zero padding between the
    // proven functions at 0x800659F0 and 0x80065B04. Scan through one 64-byte
    // alignment window so the padding is not minted as a function entry.
    constexpr uint32_t max_padding_bytes = 64;
...
bool FunctionAnalyzer::is_likely_data_section(uint32_t start_addr, uint32_t end_addr) const {
    ...
    for (uint32_t addr = start_addr; addr < end_addr; addr += 4) {
        ...
        // Subfield-aware validity (SPECIAL funct, REGIMM rt, and primary op).
        // This is the same decoder gate used by evidence-scoped discovery.
        if (!is_valid_mips_word(instr)) invalid_word_count++;
    }
    ...
    // Real PS1 code consistently uses 0% invalid words. Checking the complete
    // instruction shape (not only the primary opcode) catches numeric tables
    // whose packed halfwords happen to resemble legal J/JAL primaries.
    // A safe threshold of 7% catches all observed data sections with no false positives.
    uint32_t invalid_ratio_x100 =
        (total_words > 0) ? (invalid_word_count * 100u / total_words) : 0u;
    if (size >= 1000 && invalid_ratio_x100 > 7u) return true;
    if (size >= 100  && invalid_ratio_x100 > 50u) return true;
...
FunctionAnalysisResult FunctionAnalyzer::analyze() {
    ...
    // Subfield-aware instruction validation plus the PS1 RAM address bound for
    // direct JALs. Numeric lookup tables often decode as a run of opcode-3
    // words with impossible 0x8Fxxxxxx targets; treating only the primary
    // opcode as valid incorrectly promotes those tables to executable code.
    auto valid_word_p3 = [](uint32_t pc, uint32_t instr) -> bool {
        if (!FunctionAnalyzer::is_valid_mips_word(instr)) return false;
    ...
    auto compute_extent_p3 = [&](uint32_t entry, uint32_t hard_cap) -> ExtentP3 {
        ...
        std::queue<uint32_t> work;
        work.push(entry);
        uint32_t saved_ra_reg = 0;
        if (auto entry_word = exe_.read_word(entry); entry_word.has_value())
            saved_ra_reg = move_from_ra_destination(*entry_word);
        ...
        while (!work.empty()) {
            ...
            uint32_t instr = *word_opt;
            visited.insert(pc);
            if (!valid_word_p3(pc, instr)) { hit_invalid = true; continue; }
            if (saved_ra_reg != 0 && pc != entry &&
                instruction_writes_gpr(instr, saved_ra_reg))
                saved_ra_reg = 0;
            ExactCf cf = exact_classify_cf(pc, instr);
            uint32_t delay = pc + 4u;
            switch (cf.kind) {
            ...
            case ExactCfKind::JrOther: {
                if (in_fn(delay)) visited.insert(delay);
                uint32_t jr_rs = (instr >> 21) & 0x1Fu;
                if (saved_ra_reg != 0 && jr_rs == saved_ra_reg) {
                    reached_exit = true;
                    break;
                }
    ...
    for (size_t i = 0; i < starts_vec.size(); i++) {
        ...
        uint32_t bios_thunk_jr = 0;
        const bool bios_dispatch_thunk =
            is_bios_dispatch_thunk(func.start_addr, bios_thunk_jr);
        ExtentP3 ext = compute_extent_p3(func.start_addr, next_start);
        if (bios_dispatch_thunk) {
            // Canonical Psy-Q A0/B0/C0 call thunk. The indirect JR is an exit,
            // not an unresolved jump table, and the third instruction is its
            // delay slot. Bound the function to exactly those 12 bytes so the
            // data table that commonly follows cannot absorb the thunk.
            func.end_addr = std::min(next_start, func.start_addr + 12u);
            func.is_data_section = false;
        } else if (ext.clean_code) {
