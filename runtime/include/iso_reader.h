...
struct CDTrack {
    ...
    uint32_t start_lba;       // disc-global LBA of INDEX 01
    uint32_t file_base_lba;   // disc-global LBA corresponding to file sector 0
    uint32_t file_index_lba;  // INDEX 01 inside the referenced file
    uint32_t sector_size;     // 2048 cooked or 2352 raw/audio
    std::string file_path;    // resolved path from the cue FILE directive
};
...
private:
    ...
    struct ImageFile {
        std::string path;
        uint32_t base_lba = 0;      // disc-global LBA of file sector 0
    ...
    const CDTrack* FindTrack(uint32_t lba) const;

    bool is_open_;
    std::string volume_id_;
    std::string bin_path_;          // first data-track file (filesystem source)
    RootDirectoryInfo root_dir_;
    std::vector<CDTrack> tracks_;   // cue TOC; >=1 entry after Open()
    std::vector<std::unique_ptr<ImageFile>> image_files_;
};
