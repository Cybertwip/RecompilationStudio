...
as one of:
  ...
  - "deferred_phase5_or_trap": absent-coprocessor encodings outside verified
                               functions; retained only when Ghidra cannot prove
                               an executable path. Verified COP1 sites use the
                               Beetle-matched COPU/NOP-like implementation.
...
IMPLEMENTED = {
    ...
    # COP1 absent-hardware semantics (Ghidra-proven BIOS strtod function).
    "17|rs_cop0|4":  ("MTC1/COP1 move", f"{TRANSLATOR_PATH} primary 0x11: SR.CU1 clear -> COPU; set -> NOP-like"),
    "17|rs_cop0|8":  ("BC1", f"{TRANSLATOR_PATH} primary 0x11 rs 0x08: COPU or hardwired-false delayed branch"),
    "17|rs_cop0|17": ("COP1 fmt D", f"{TRANSLATOR_PATH} primary 0x11 fmt D: COPU or NOP-like"),
    "17|rs_cop0|20": ("COP1 fmt W", f"{TRANSLATOR_PATH} primary 0x11 fmt W: COPU or NOP-like"),
    "49|none|0":     ("LWC1", f"{TRANSLATOR_PATH} primary 0x31: COPU or aligned discarded read"),
...
DEFERRED = {
    # COP1/COP3 encodings not associated with a Ghidra-proven function remain
    # deferred as data until control-flow evidence promotes them. The verified
    # BFC02324 COP1/LWC1 sites are implemented above with hardware trap semantics.
    "17|none|0": ("COP1", "Unclassified COP1 bucket; require Ghidra control-flow proof before promotion."),
