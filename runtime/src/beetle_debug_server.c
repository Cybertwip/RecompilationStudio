...
#ifdef PSX_BEETLE_EXC_RING
extern int retro_psxref_exc_ring_dump(char *out, int cap);
#endif
static void h_exc_ring(int id, const char *json) {
    (void)json;
#ifndef PSX_BEETLE_EXC_RING
    send_err(id, "exc_ring unsupported by this Beetle core");
#else
    int cap = 4 * 1024 * 1024;
 ...
 * Tap 0 is Beetle's fully-mixed 44.1 kHz SPU output; tap 2 is the PCM pulled
 * by the host audio device after the bounded DRC bridge. */
static void h_audio_stats(int id, const char *json) {
    (void)json;
    AudioTraceStats st;
    audio_trace_get_stats(&st);
    extern int beetle_host_audio_stats(double*, uint64_t*, uint64_t*, double*, int*);
    double fill_ms = 0.0, correction = 0.0;
    uint64_t bridge_underruns = 0, bridge_overflows = 0;
    int host_rate = 0;
    int bridge_ready = beetle_host_audio_stats(
        &fill_ms, &bridge_underruns, &bridge_overflows, &correction, &host_rate);
    send_fmt("{\"id\":%d,\"ok\":true,"
             "\"taps\":["
             ...
             "\"bridge\":{\"ready\":%d,\"host_rate\":%d,\"fill_ms\":%.3f,"
             "\"correction\":%.7f,\"underruns\":%llu,\"overflow_drops\":%llu}}\n",
             id,
             ...
             bridge_ready, host_rate, fill_ms, correction,
             (unsigned long long)bridge_underruns,
             (unsigned long long)bridge_overflows);
}
