...
static void text_guard_activate_baseline(void);
...
/* False until the BIOS has loaded the game EXE and fntrace establishes the
 * game-start baseline. Before that point the same RAM pages legitimately hold
 * BIOS shell code, so a byte mismatch must reject native dispatch WITHOUT
 * permanently poisoning the page. */
static int text_image_active = 0;
...
int dirty_ram_text_image_active(void) { return text_image_active; }
...
int dirty_ram_text_native_ok(uint32_t phys) {
    ...
    uint32_t page = phys >> DIRTY_RAM_PAGE_SHIFT;
    uint32_t bit = 1u << (page & 31u);
    if (text_image_active && (text_diverged_bitmap[page >> 5] & bit)) {
    ...
    /* Before game handoff this mismatch is expected: BIOS shell code occupies
     * the future game pages. Reject native execution for now, but do not make
     * the result sticky. Once the game baseline is active, a mismatch is a real
     * self-modification/overlay and remains sticky exactly as before. */
    if (text_image_active) {
...
/* Coprocessor loads: §1/DO_LDS is charged by psx_cyc_step(); this helper adds
 * the address-keyed ReadMemory completion without arming a GPR load delay. */
uint32_t psx_cyc_cop_load_word(CPUState* cpu, uint32_t addr) {
#ifdef PSX_ENABLE_BLOCK_CYCLES
...
uint32_t psx_cyc_lwc2_read(CPUState* cpu, uint32_t addr) {
    return psx_cyc_cop_load_word(cpu, addr);
}
