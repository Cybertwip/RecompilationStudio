...
/* Hardware polygon size rejection (psx-spx, GPU polygon commands): each
 * triangle is discarded when its X extent exceeds 1023 or Y extent exceeds
 * 511. This must live in the shared GP0 layer so software, OpenGL, and Vulkan
 * backends all receive the same accepted primitive stream. GTE near-plane
 * saturation commonly produces +/-1024 vertices; without this gate those
 * close-camera polygons become screen-covering garbage. */
uint64_t g_prim_size_rejects = 0;
static int tri_size_ok(int32_t x0, int32_t y0,
                       int32_t x1, int32_t y1,
                       int32_t x2, int32_t y2) {
...
static void gp0_exec_mono_tri(void) {
    ...
    int tri_ok = tri_size_ok(vx[0], vy[0], vx[1], vy[1], vx[2], vy[2]);
...
static void gp0_exec_mono_quad(void) {
    ...
    int tri0_ok = tri_size_ok(vx[0], vy[0], vx[1], vy[1], vx[2], vy[2]);
    int tri1_ok = tri_size_ok(vx[2], vy[2], vx[1], vy[1], vx[3], vy[3]);
    if (tri0_ok && tri1_ok &&
        vx[0] == vx[2] && vx[1] == vx[3] &&
        vy[0] == vy[1] && vy[2] == vy[3] &&
        vx[1] > vx[0] && vy[2] > vy[0] &&
