...
#undef XRES
    ...
    case 0x11: { /* COP1 absent on retail PS1 */
        uint32_t cop_op = rs;
        ...
        if (cop_op == 0x08 || cop_op == 0x0C) {
            int taken = (insn & (1u << 16)) == 0;
            exec_delay_slot(cpu, pc + 4);
            cosim_exec_one_transfer_hook(pc + 4);
            cpu->pc = taken ? (pc + 4 + (simm << 2)) : (pc + 8);
            return 1;
        }
        return 0; /* CU1-enabled non-branch COP1 is NOP-like */
    }
    case 0x12: { /* COP2 / GTE */
        uint32_t cop_op = rs;
    ...
    case 0x31: { /* LWC1 — CU1-enabled read is discarded */
        uint32_t addr = cpu->gpr[rs] + (uint32_t)simm;
        if (!(cpu->cop0[12] & (1u << 29))) { psx_cop_unusable(cpu, 1u, pc); return 1; }
        if (addr & 3u) { psx_address_error_exception(cpu, addr, pc, 0); return 1; }
        (void)psx_cyc_cop_load_word(cpu, addr);
        return 0;
    }
    case 0x39: { /* SWC1 — CU1-enabled operation performs no write */
        uint32_t addr = cpu->gpr[rs] + (uint32_t)simm;
        if (!(cpu->cop0[12] & (1u << 29))) { psx_cop_unusable(cpu, 1u, pc); return 1; }
        if (addr & 3u) { psx_address_error_exception(cpu, addr, pc, 1); return 1; }
        return 0;
    }
