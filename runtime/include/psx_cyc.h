...
#if defined(_MSC_VER)
        unsigned long n;
        _BitScanForward(&n, (unsigned long)reg_mask);
#else
...
/* Coprocessor load ReadMemory timing only (completion +1, NO LDWhich arm — the
 * destination is not a GPR). Call AFTER psx_cyc_step() has charged §1+DO_LDS.
 * LWC2 additionally stalls to the GTE deadline before using the same read. */
extern uint32_t psx_cyc_cop_load_word(CPUState* cpu, uint32_t addr);
