...
int main(int argc, char** argv) {
    ...
    std::vector<PSXRecomp::Function> classified_data_ranges;
    ...
    // Whole-image discovery intentionally scans aggressively, so data tables
    // can look like function starts (embedded jr-ra words, pointer-shaped
    // values, etc.). A data classification is NOT a function implementation:
    // never emit a fatal-dispatch body for it. Preserve the classification as
    // a machine-readable manifest, reject any explicit seed that landed in one
    // of the ranges, and remove the ranges before CFG/code generation.
    if (!overlay_mode) {
        for (const auto& func : analysis_result.functions) {
            if (func.is_data_section) classified_data_ranges.push_back(func);
        }

        auto seeded_in_data = [&](uint32_t seed) -> const PSXRecomp::Function* {
            for (const auto& data : classified_data_ranges) {
                if (seed >= data.start_addr && seed < data.end_addr) return &data;
            }
            return nullptr;
        };
        for (uint32_t seed : file_seeds) {
            if (const auto* data = seeded_in_data(seed)) {
                fmt::print(stderr,
                           "ERROR: explicit seed 0x{:08X} was classified as data "
                           "range 0x{:08X}..0x{:08X}; fix the seed/discovery "
                           "evidence instead of emitting a stub.\n",
                           seed, data->start_addr, data->end_addr);
                return 1;
            }
        }
        analysis_result.functions.erase(
            std::remove_if(analysis_result.functions.begin(),
                           analysis_result.functions.end(),
                           [](const PSXRecomp::Function& func) {
                               return func.is_data_section;
                           }),
            analysis_result.functions.end());

        if (!classified_data_ranges.empty()) {
            fmt::print("Classified {} non-code data range(s); omitted from "
                       "function/dispatch generation.\n\n",
                       classified_data_ranges.size());
        }
    }
    ...
    // A JAL into bytes proven to be data is runtime-installed code: the program
    // must replace those bytes before executing the call target. Keep the data
    // out of native C and let the generated call route through runtime dispatch,
    // which accepts the target only after dirty-RAM tracking proves it was
    // written. Branches and plain jumps into data remain hard failures because
    // they do not carry the call/return contract that identifies this pattern.
    std::vector<DynamicDataTarget> dynamic_data_targets;
    if (!classified_data_ranges.empty()) {
        auto target_data = [&](uint32_t target) -> const PSXRecomp::Function* {
            for (const auto& data : classified_data_ranges) {
                if (target >= data.start_addr && target < data.end_addr) return &data;
            }
            return nullptr;
        };
        for (const auto& [func_addr, cfg] : all_cfgs) {
            for (const auto& [block_addr, block] : cfg.blocks) {
                const auto type = block.exit_instr.type;
                if (type != PSXRecomp::ControlFlowType::Branch &&
                    type != PSXRecomp::ControlFlowType::Jump &&
                    type != PSXRecomp::ControlFlowType::JumpLink) {
                    continue;
                }
                const uint32_t target = block.exit_instr.target;
                if (target == 0) continue;
                if (const auto* data = target_data(target)) {
                    if (type == PSXRecomp::ControlFlowType::JumpLink) {
                        dynamic_data_targets.push_back({
                            block.exit_instr.address, target,
                            data->start_addr, data->end_addr});
                        continue;
                    }
                    fmt::print(stderr,
                               "ERROR: control flow 0x{:08X} -> 0x{:08X} "
                               "enters classified data range "
                               "0x{:08X}..0x{:08X}; classification is not "
                               "proven safe.\n",
                               block.exit_instr.address, target,
                               data->start_addr, data->end_addr);
                    return 1;
    ...
    std::sort(dynamic_data_targets.begin(), dynamic_data_targets.end(),
              [](const DynamicDataTarget& a, const DynamicDataTarget& b) {
                  return std::tie(a.source, a.target) < std::tie(b.source, b.target);
              });
    dynamic_data_targets.erase(
        std::unique(dynamic_data_targets.begin(), dynamic_data_targets.end(),
                    [](const DynamicDataTarget& a, const DynamicDataTarget& b) {
                        return a.source == b.source && a.target == b.target;
                    }),
        dynamic_data_targets.end());
    if (!dynamic_data_targets.empty()) {
        fmt::print("Classified {} JAL target(s) as runtime-installed code; "
                   "native dispatch is omitted and dirty-RAM proof is required.\n\n",
                   dynamic_data_targets.size());
    }
    ...
    // Data-classification proof artifact. These ranges are deliberately absent
    // from generated C, the range manifest, and the dispatch table.
    {
        std::filesystem::path data_filename =
            out_dir / (exe_stem + "_data_ranges.json");
        std::ofstream df(data_filename);
        if (!df.is_open()) {
            fmt::print(stderr, "ERROR: failed to write data-range manifest {}\n",
                       data_filename.string());
            return 1;
        }
        uint64_t total_bytes = 0;
        for (const auto& data : classified_data_ranges) total_bytes += data.size;
        df << "{\n";
        df << "  \"schema\": 1,\n";
        df << "  \"classifier\": \"FunctionAnalyzer::is_data_section\",\n";
        df << fmt::format("  \"range_count\": {},\n",
                          classified_data_ranges.size());
        df << fmt::format("  \"total_bytes\": {},\n", total_bytes);
        df << "  \"ranges\": [\n";
        for (size_t i = 0; i < classified_data_ranges.size(); ++i) {
            const auto& data = classified_data_ranges[i];
            df << fmt::format(
                "    {{\"start\":\"0x{:08X}\",\"end\":\"0x{:08X}\","
                "\"bytes\":{}}}{}\n",
                data.start_addr, data.end_addr, data.size,
                (i + 1 == classified_data_ranges.size()) ? "" : ",");
        }
        df << "  ]\n";
        df << "}\n";
        df.close();
        fmt::print("✓ Saved data-range manifest to {}\n\n",
                   data_filename.string());
    }

    {
        std::filesystem::path dynamic_filename =
            out_dir / (exe_stem + "_dynamic_targets.json");
        std::ofstream dy(dynamic_filename);
        if (!dy.is_open()) {
            fmt::print(stderr, "ERROR: failed to write dynamic-target manifest {}\n",
                       dynamic_filename.string());
            return 1;
        }
        dy << "{\n";
        dy << "  \"schema\": 1,\n";
        dy << "  \"classification\": \"JAL into omitted data; runtime write required\",\n";
        dy << fmt::format("  \"target_count\": {},\n", dynamic_data_targets.size());
        dy << "  \"targets\": [\n";
        for (size_t i = 0; i < dynamic_data_targets.size(); ++i) {
            const auto& target = dynamic_data_targets[i];
            dy << fmt::format(
                "    {{\"source\":\"0x{:08X}\",\"target\":\"0x{:08X}\","
                "\"data_start\":\"0x{:08X}\",\"data_end\":\"0x{:08X}\","
                "\"dispatch\":\"dirty_ram_required\"}}{}\n",
                target.source, target.target, target.data_start, target.data_end,
                (i + 1 == dynamic_data_targets.size()) ? "" : ",");
        }
        dy << "  ]\n";
        dy << "}\n";
        dy.close();
        fmt::print("✓ Saved dynamic-target manifest to {}\n\n",
                   dynamic_filename.string());
    }
    ...
    // build; only the overlay path actually loads it.
    {
        ...
        std::string ranges = codegen.generate_emitted_ranges_manifest();
