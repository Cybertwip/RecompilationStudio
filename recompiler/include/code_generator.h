...
public:
    ...
    // Generate ranges for the exact post-split function/CFG set used by the
    // most recent generate_file() call. This prevents the manifest from
    // omitting mid-function entries created by the codegen pre-pass.
    std::string generate_emitted_ranges_manifest();
...
private:
    ...
    std::vector<Function> emitted_functions_;
    std::map<uint32_t, ControlFlowGraph> emitted_cfgs_;
    bool emitted_snapshot_valid_ = false;
