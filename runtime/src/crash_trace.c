...
void psx_crash_trace_dump(const char *reason, void *seh_info) {
    ...
    char reason_json[4096];
    char origin_json[256];
    json_escape(reason ? reason : "(unknown)", reason_json, sizeof(reason_json));
    json_escape(s_exit_origin, origin_json, sizeof(origin_json));
