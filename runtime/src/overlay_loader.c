...
int overlay_loader_dispatch(CPUState *cpu, uint32_t addr) {
    /* The dynamic overlay layer is optional. When [runtime].overlay_cache is
     * false, overlay_loader_init() is never called and the range-index heads
     * retain C's zero initialization rather than the -1 sentinel installed by
     * init. Entering the CPS range lookup in that state follows link 0 back to
     * itself forever (observed on Evil Zone's first BIOS C0 dispatch, phys
     * 0xC0). The loader contract is a strict no-op until initialization. */
    if (!s_active) return 0;

    uint32_t phys = addr & 0x1FFFFFFFu;
    int lazy_loaded = 0;
retry_candidates:
    ; /* C99: a label must precede a statement, not a declaration. */
...
int overlay_loader_dump_lazy_at(uint32_t addr, char *out, int cap) {
    if (!s_active) return snprintf(out, cap, "{\"active\":false,\"entries\":[]}");
    uint32_t phys = addr & 0x1FFFFFFFu;
    uint32_t pg = phys >> 12;
