...
static rab_bridge s_audio_drc;
static int s_audio_drc_ready;
static int s_audio_host_rate = 44100;

static void beetle_audio_callback(void*, Uint8* stream, int len) {
    int frames = len / (int)(2 * sizeof(int16_t));
    if (!s_audio_drc_ready) {
        std::memset(stream, 0, (size_t)len);
        return;
    }
    rab_pull(&s_audio_drc, reinterpret_cast<int16_t*>(stream), frames);
...
extern "C" void beetle_host_audio_push(const int16_t *stereo, size_t frames) {
    if (!s_audio_device || !s_audio_drc_ready || !stereo || frames == 0) return;
    SDL_LockAudioDevice(s_audio_device);
    rab_push(&s_audio_drc, stereo, (int)frames);
    SDL_UnlockAudioDevice(s_audio_device);
}

extern "C" int beetle_host_audio_stats(double *fill_ms, uint64_t *underruns,
                                         uint64_t *overflow_drops,
                                         double *correction, int *host_rate) {
    if (host_rate) *host_rate = s_audio_host_rate;
    if (!s_audio_drc_ready) return 0;
    rab_stats st;
    rab_get_stats(&s_audio_drc, &st);
    if (fill_ms) *fill_ms = st.last_fill_ms;
...
int main(int argc, char** argv) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO | (host_audio ? SDL_INIT_AUDIO : 0)) != 0) {
        std::fprintf(stderr, "SDL_Init failed: %s\n", SDL_GetError());
        return 1;
    }

    SDL_Window* win = SDL_CreateWindow(
        "ORACLE EMULATOR — psx-beetle (not the recomp port)",
    ...
    if (host_audio) {
        ...
        want.channels = 2;
        want.samples = 1024;
        want.callback = beetle_audio_callback;
        s_audio_device = SDL_OpenAudioDevice(NULL, 0, &want, &have,
        ...
        if (have.format != AUDIO_S16SYS || have.channels != 2) {
            std::fprintf(stderr,
                         "SDL audio device did not accept stereo S16 "
                         "(got %d Hz, %u channels, format 0x%X)\n",
                         have.freq, (unsigned)have.channels, (unsigned)have.format);
            return 1;
        }
        rab_config cfg;
        rab_config_defaults(&cfg);
        cfg.channels = 2;
        cfg.source_rate = 44100.0;
        cfg.host_rate = (double)have.freq;
        cfg.target_ms = 80.0;
        cfg.ring_ms = 250.0;
        cfg.em_high_ms = 205.0;
        if (rab_init(&s_audio_drc, &cfg) != 0) {
            std::fprintf(stderr, "audio DRC bridge initialization failed\n");
            return 1;
        }
        s_audio_drc_ready = 1;
        s_audio_host_rate = have.freq;
        audio_trace_set_tap_rate(AUDIO_TAP_HOST, (uint32_t)have.freq);
        SDL_PauseAudioDevice(s_audio_device, 0);
        std::fprintf(stdout,
                     "psx-beetle: host audio enabled (source 44100 Hz, device %d Hz, "
                     "DRC target 80 ms)\n", have.freq);
    }
    ...
    for (;;) {
...
#endif
                beetle_shutdown();
                if (s_audio_device) SDL_CloseAudioDevice(s_audio_device);
                if (s_audio_drc_ready) rab_free(&s_audio_drc);
        ...
        const Uint8* keys = SDL_GetKeyboardState(NULL);
        if (frontend_turbo || (keys && keys[SDL_SCANCODE_TAB])) continue;
