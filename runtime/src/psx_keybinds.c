...
static void derive_ini_path(const char *exe_path) {
    const char *runtime_data_dir = getenv("PSX_RUNTIME_DATA_DIR");
    if (runtime_data_dir && runtime_data_dir[0]) {
        size_t len = strlen(runtime_data_dir);
        const char *sep = (len > 0 &&
            (runtime_data_dir[len - 1] == '/' || runtime_data_dir[len - 1] == '\\'))
            ? "" : "/";
        snprintf(s_ini_path, sizeof(s_ini_path), "%s%skeybinds.ini",
                 runtime_data_dir, sep);
        return;
    }
    /* SDL knows the real executable directory even when argv[0] is an
     * extensionless path such as ./build/PepsiManRecomp. The old extension
     * heuristic treated that executable as a directory and attempted to write
     * .../PepsiManRecomp/keybinds.ini, which can never exist. */
    char *base = SDL_GetBasePath();
    if (base && *base) {
        snprintf(s_ini_path, sizeof(s_ini_path), "%skeybinds.ini", base);
        SDL_free(base);
        return;
    }
...
uint16_t psx_keybinds_button_mask_for_scancode(SDL_Scancode sc, int player) {
    if (sc == SDL_SCANCODE_UNKNOWN) return 0;
    const PsxPlayerBinds *pb = player_binds_c(player);
    uint16_t mask = 0;
    for (int i = 0; i < PSXKB_N; i++) {
        if (!s_buttons[i].bit) continue;
        SDL_Scancode bound = *(const SDL_Scancode *)((const char *)pb + s_buttons[i].offset);
        if (bound == sc ||
            (i == PSX_KB_START && bound == SDL_SCANCODE_RETURN &&
             sc == SDL_SCANCODE_KP_ENTER))
            mask |= s_buttons[i].bit;
    }
    return mask;
}
