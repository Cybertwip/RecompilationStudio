 ...
 * This file is compiled only into explicit oracle targets; native targets do
 * not link any implementation of this general interpreter API.
 */
...
static void exec_one(CPUState* cpu) {
    ...
    /* --- COP1 (absent on retail PS1) --- */
    case 0x11: {
        if (!(cpu->cop0[12] & (1u << 29))) {
            psx_cop_unusable(cpu, 1u, pc);
            return;
        }
        uint32_t cop_op = RS(insn);
        if (cop_op == 0x08 || cop_op == 0x0C) {
            int taken = (insn & (1u << 16)) == 0; /* condition is hardwired false */
            uint32_t target = pc + 4 + (SIMM(insn) << 2);
            exec_one(cpu); /* delay slot */
            if (cpu->pc != pc + 8) return; /* delay-slot exception/transfer wins */
            cpu->pc = taken ? target : pc + 8;
            return;
        }
        /* With CU1 set, Beetle models all other COP1 operations as NOP-like. */
        break;
    }
    ...
    /* --- Absent COP1 load/store --- */
    case 0x31: { /* LWC1: read and discard only when CU1 is enabled */
        uint32_t addr = rs_val + (uint32_t)SIMM(insn);
        if (!(cpu->cop0[12] & (1u << 29))) { psx_cop_unusable(cpu, 1u, pc); return; }
        if (addr & 3u) { psx_address_error_exception(cpu, addr, pc, 0); return; }
        (void)cpu->read_word(addr);
        break;
    }
    case 0x39: { /* SWC1: alignment check only, no memory write */
        uint32_t addr = rs_val + (uint32_t)SIMM(insn);
        if (!(cpu->cop0[12] & (1u << 29))) { psx_cop_unusable(cpu, 1u, pc); return; }
        if (addr & 3u) { psx_address_error_exception(cpu, addr, pc, 1); return; }
        break;
    }
