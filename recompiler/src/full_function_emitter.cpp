...
static std::string branch_condition(const char* kind, uint32_t branch_addr) {
    ...
    if (k == "branch_bc1f" || k == "branch_bc1t")
        return fmt::format("psx_brA_{:08X} != 0", branch_addr);
    return "";
}
    ...
    std::vector<ContinuationLabel>& out_cross_targets)
{
    ...
    for (const auto& [addr, raw] : addr_to_raw) {
        ...
        if (tr.is_terminator) {
            ...
            if (k == "rfe" || k == "reserved_instruction") {
                continue; /* synchronous no-delay exception/return */
            }
    ...
    for (auto it = addr_to_raw.begin(); it != addr_to_raw.end(); ++it) {
        ...
        if (tr.is_terminator) {
            ...
            } else if (kind == "reserved_instruction") {
                out += fmt::format("    {}\n", tr.c_code);
            }
            if (kind != "rfe" && kind != "reserved_instruction" &&
                !orphaned_delay_slots.count(addr)) {
            ...
            if (orphaned_delay_slots.count(addr)) {
                if (kind == "jr") {
                    // A JR still owns and must execute its delay slot before
                    // publishing the jump target. Function discovery can split
                    // that slot into an adjacent seeded function (for example,
                    // a stack-restoring `addiu $sp, $sp, N` misidentified as a
                    // function start). Returning first skips the side effect and
                    // corrupts the caller's frame. Inline the ROM instruction
                    // here, just as the orphaned J/JAL/JALR paths do below.
                    uint32_t ds_addr = addr + 4;
                    ...
                    if (ds_phys < base_phys || ds_phys + 4 > base_phys + rom.size()) {
                        out += fmt::format("    /* UNSUPPORTED orphaned JR delay 0x{:08X}: outside ROM */\n",
                                           ds_addr);
                        return false;
                    }
                    ...
                    if (!ds_tr.supported) {
                        out += fmt::format("    /* UNSUPPORTED orphaned JR delay 0x{:08X}: {} */\n",
                                           ds_addr, ds_tr.fail_reason);
                        return false;
                    }
                    ...
                    const std::string ds_kind = ds_tr.terminator_kind
                        ? ds_tr.terminator_kind : "";
                    if (!ds_tr.is_terminator) {
                        out += fmt::format("    {}\n", ds_tr.c_code);
                    } else if (ds_kind == "rfe") {
                        // RFE is valid as the delay slot of the exception
                        // handler's JR. Apply the SR stack pop without using the
                        // translator's standalone-return form; the JR below owns
                        // the actual transfer.
                        out += "    { uint32_t sr = cpu->cop0[12]; "
                               "cpu->cop0[12] = (sr & 0xFFFFFFC0u) | ((sr >> 2) & 0x0Fu); } "
                               "/* rfe (orphaned delay) */\n";
                        out += "    psx_rfe_mark_escape();\n";
                    } else if (ds_kind == "reserved_instruction") {
                        // The synchronous RI exception prevents the pending JR
                        // from completing; the helper returns from this C frame.
                        out += fmt::format("    {}\n", ds_tr.c_code);
                    } else {
                        out += fmt::format("    /* UNSUPPORTED control transfer in orphaned JR delay "
                                           "0x{:08X}: {} */\n", ds_addr, ds_kind);
                        return false;
                    }
    ...
    const std::vector<BiosAlias>&       bios_aliases)
{
    ...
    if (cps) {
        ...
        out += "static void __cdecl psx_cps_mark_bios(void);\n";
        out += "#pragma section(\".CRT$XCU\", read)\n";
        out += "__declspec(allocate(\".CRT$XCU\")) static void (__cdecl *psx_cps_mark_bios_ctor)(void) = psx_cps_mark_bios;\n";
        out += "static void __cdecl psx_cps_mark_bios(void) {\n";
        out += "#else\n";
