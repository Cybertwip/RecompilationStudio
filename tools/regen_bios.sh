...
if [ -n "${PSXRECOMP_BIOS_ROM:-}" ]; then
    BIOS="$PSXRECOMP_BIOS_ROM"
elif [ -f bios/SCPH1001.BIN ]; then
    BIOS="bios/SCPH1001.BIN"
elif [ -f branding/SCPH1001.BIN ]; then
    BIOS="branding/SCPH1001.BIN"
else
    echo "regen_bios: canonical SCPH1001.BIN not found in bios/ or branding/"
    exit 1
fi
...
bash "$ROOT/tools/bios_emitter_fingerprint.sh" > "$OUT/SCPH1001.emitter.sha"
