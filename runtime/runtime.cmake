...
if(PSX_VIRTUA)
    include("${PSXRECOMP_ROOT}/extra/virtua/CMake/VirtuaPowerEngine.cmake")
...
# macOS direct Xbox GIP backend. SDL cannot expose several wired PDP/Microsoft/
# PowerA Xbox devices on macOS because they speak the vendor-specific Game Input
# Protocol rather than standard USB HID. libusb is a build-time dependency; when
# its static archive is available it is folded into the runtime so end users do
# not need a Homebrew dylib installed.
set(PSXRECOMP_GIP_GAMEPAD_AVAILABLE OFF)
if(APPLE)
    option(PSX_MACOS_GIP_GAMEPAD
           "Build direct USB support for Xbox GIP controllers on macOS" ON)
endif()
if(APPLE AND PSX_MACOS_GIP_GAMEPAD)
    find_package(PkgConfig REQUIRED)
    pkg_check_modules(PSXRECOMP_LIBUSB QUIET IMPORTED_TARGET libusb-1.0)
    if(PSXRECOMP_LIBUSB_FOUND)
        set(PSXRECOMP_GIP_GAMEPAD_AVAILABLE ON)
        foreach(_psx_libusb_dir IN LISTS PSXRECOMP_LIBUSB_LIBRARY_DIRS)
            if(EXISTS "${_psx_libusb_dir}/libusb-1.0.a")
                set(PSXRECOMP_LIBUSB_STATIC_ARCHIVE
                    "${_psx_libusb_dir}/libusb-1.0.a")
                break()
            endif()
        endforeach()
        if(PSXRECOMP_LIBUSB_STATIC_ARCHIVE)
            find_library(PSXRECOMP_IOKIT_FRAMEWORK IOKit REQUIRED)
            find_library(PSXRECOMP_COREFOUNDATION_FRAMEWORK CoreFoundation REQUIRED)
            find_library(PSXRECOMP_SECURITY_FRAMEWORK Security REQUIRED)
            message(STATUS "psxrecomp: macOS Xbox GIP support (static libusb)")
        else()
            message(STATUS "psxrecomp: macOS Xbox GIP support (shared libusb)")
        endif()
    else()
        message(WARNING
            "psxrecomp: libusb-1.0 was not found; wired Xbox GIP controllers "
            "that SDL cannot enumerate will be unavailable on macOS. Install "
            "libusb and reconfigure to build the direct-USB backend.")
    endif()
endif()
...
set(PSXRECOMP_RUNTIME_SOURCES
    ...
    ${PSXRECOMP_ROOT}/runtime/src/pad_negotiation.c
...
function(psxrecomp_configure_gip_gamepad target)
    if(NOT PSXRECOMP_GIP_GAMEPAD_AVAILABLE)
        return()
    endif()
    target_compile_definitions(${target} PRIVATE PSX_HAVE_GIP_GAMEPAD=1)
    target_include_directories(${target} PRIVATE ${PSXRECOMP_LIBUSB_INCLUDE_DIRS})
    if(PSXRECOMP_LIBUSB_STATIC_ARCHIVE)
        target_link_libraries(${target} PRIVATE
            "${PSXRECOMP_LIBUSB_STATIC_ARCHIVE}"
            "${PSXRECOMP_IOKIT_FRAMEWORK}"
            "${PSXRECOMP_COREFOUNDATION_FRAMEWORK}"
            "${PSXRECOMP_SECURITY_FRAMEWORK}"
            objc)
    else()
        target_link_libraries(${target} PRIVATE PkgConfig::PSXRECOMP_LIBUSB)
    endif()
endfunction()

if(NOT DEFINED PSXRECOMP_BIOS_STEM)
    ...
    if(_psxrt_bash AND EXISTS "${PSXRECOMP_ROOT}/tools/bios_emitter_fingerprint.sh")
        ...
        if(_psxrt_fp_rc EQUAL 0 AND _psxrt_cur_fp)
            ...
            if(NOT _psxrt_saved_fp STREQUAL _psxrt_cur_fp)
                message(WARNING
                    "BIOS ${PSXRECOMP_BIOS_STEM} generated/ is STALE vs the recompiler emitter "
...
function(psxrecomp_add_runtime_target target)
    ...
    if(PSXRT_BIOS_GENERATED_FULL_C OR PSXRT_BIOS_GENERATED_DISPATCH_C)
        if(NOT PSXRT_BIOS_GENERATED_FULL_C OR NOT PSXRT_BIOS_GENERATED_DISPATCH_C)
            message(FATAL_ERROR
                "psxrecomp_add_runtime_target: BIOS_GENERATED_FULL_C and "
                "BIOS_GENERATED_DISPATCH_C must be supplied together")
        endif()
        set(_psxrt_bios_generated
            "${PSXRT_BIOS_GENERATED_FULL_C}"
            "${PSXRT_BIOS_GENERATED_DISPATCH_C}")
    else()
        set(_psxrt_bios_generated ${PSXRECOMP_BIOS_GENERATED})
    endif()
    foreach(_psxrt_bios_source IN LISTS _psxrt_bios_generated)
        set_source_files_properties("${_psxrt_bios_source}" PROPERTIES GENERATED TRUE)
    endforeach()
    set(generated_sources ${_psxrt_bios_generated})
    ...
    if(NOT has_game_dispatch)
        list(APPEND generated_sources
            "${PSXRECOMP_ROOT}/runtime/src/no_game_text.c")
    endif()
    ...
    if(PSXRT_ORACLE)
        set(mode_sources ${PSXRECOMP_ROOT}/runtime/src/psx_interpreter.c)
    else()
        set(mode_sources)
    endif()
    ...
    psxrecomp_configure_gip_gamepad(${target})
    ...
    # read the same generated PSX_OVERLAY_CODEGEN_HASH, so they never drift. 
    # Idempotent write avoids rebuilds.
    ...
    target_compile_definitions(${target} PRIVATE
        ...
        PSX_APP_SUPPORT_DIR_NAME="${PSXRT_APP_SUPPORT_DIR_NAME}"
    ...
    if(PSX_VIRTUA)
        ...
        target_compile_options(${target} PRIVATE
            ...
            $<$<COMPILE_LANGUAGE:CXX>:-isystem${VIRTUA_ARM_CXX_INCLUDE}>
        ...
        target_link_directories(${target} PRIVATE
            ${VIRTUA_ARM_SYSROOT}/lib
            ${VIRTUA_ARM_SYSROOT}/lib/${VIRTUA_ARM_TARGET_TRIPLE})
