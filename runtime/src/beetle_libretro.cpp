...
/* ---- Per-PC guest-cycle watch (oracle half of tools/cycle_compare.py) ---- */
#define BEETLE_CYC_WATCH_CAP 1024u
struct BeetleCycWatchEntry {
    uint32_t hit_index;
    uint32_t pc;
    uint64_t cycles;
};
static BeetleCycWatchEntry s_cyc_watch[BEETLE_CYC_WATCH_CAP];
static uint32_t s_cyc_anchor_raw, s_cyc_anchor_phys;
static uint32_t s_cyc_end_raw, s_cyc_end_phys;
static uint32_t s_cyc_max_hits, s_cyc_hits;
static uint64_t s_cyc_region_start;
static int s_cyc_armed, s_cyc_in_region;

extern "C" unsigned long long beetle_core_get_guest_cycles(void);

static void cyc_watch_cpu_hook(const pscpu_timestamp_t timestamp, uint32 pc) {
    if (!s_cyc_armed) return;
    uint32_t phys = pc & 0x1FFFFFFFu;
    uint64_t now = (uint64_t)beetle_core_get_guest_cycles() + (uint32_t)timestamp;
    if (s_cyc_end_phys != 0u) {
        if (!s_cyc_in_region) {
            if (phys == s_cyc_anchor_phys) {
                s_cyc_region_start = now;
                s_cyc_in_region = 1;
            }
        } else if (phys == s_cyc_end_phys) {
            BeetleCycWatchEntry &e = s_cyc_watch[s_cyc_hits];
            e.hit_index = s_cyc_hits;
            e.pc = phys;
            e.cycles = now - s_cyc_region_start;
            s_cyc_hits++;
            s_cyc_in_region = 0;
        }
    } else if (phys == s_cyc_anchor_phys) {
        BeetleCycWatchEntry &e = s_cyc_watch[s_cyc_hits];
        e.hit_index = s_cyc_hits;
        e.pc = phys;
        e.cycles = now;
    ...
    if (s_cyc_hits >= s_cyc_max_hits) {
        s_cyc_armed = 0;
        if (PSX_CPU) PSX_CPU->SetCPUHook(nullptr, nullptr);
    }
}

extern "C" void beetle_cyc_watch_arm(uint32_t anchor_raw, uint32_t end_raw, int n) {
    if (n < 1) n = 1;
    if (n > (int)BEETLE_CYC_WATCH_CAP) n = (int)BEETLE_CYC_WATCH_CAP;
    s_cyc_armed = 0;
    s_cyc_anchor_raw = anchor_raw;
    s_cyc_anchor_phys = anchor_raw & 0x1FFFFFFFu;
    s_cyc_end_raw = end_raw;
    s_cyc_end_phys = end_raw ? (end_raw & 0x1FFFFFFFu) : 0u;
    s_cyc_max_hits = (uint32_t)n;
    s_cyc_hits = 0;
    s_cyc_in_region = 0;
    s_cyc_region_start = 0;
    std::memset(s_cyc_watch, 0, sizeof(s_cyc_watch));
    s_cyc_armed = 1;
    if (PSX_CPU) PSX_CPU->SetCPUHook(cyc_watch_cpu_hook, nullptr);
}
...
extern "C" void beetle_cyc_watch_get_state(
    uint32_t *anchor_raw, uint32_t *anchor_phys,
    uint32_t *end_raw, uint32_t *end_phys,
    uint32_t *max_hits, uint32_t *hits, int *armed) {
    if (anchor_raw) *anchor_raw = s_cyc_anchor_raw;
    if (anchor_phys) *anchor_phys = s_cyc_anchor_phys;
    if (end_raw) *end_raw = s_cyc_end_raw;
    if (end_phys) *end_phys = s_cyc_end_phys;
    if (max_hits) *max_hits = s_cyc_max_hits;
    if (hits) *hits = s_cyc_hits;
    if (armed) *armed = s_cyc_armed;
}

extern "C" int beetle_cyc_watch_get(uint32_t i, uint32_t *hit_index,
                                      uint32_t *pc, unsigned long long *cycles) {
    if (i >= s_cyc_hits) return 0;
    const BeetleCycWatchEntry &e = s_cyc_watch[i];
    if (hit_index) *hit_index = e.hit_index;
    if (pc) *pc = e.pc;
    if (cycles) *cycles = (unsigned long long)e.cycles;
    return 1;
}
...
/* Host playback sink owned by beetle_main.cpp. The diagnostic SPU_OUT tap is
 * recorded independently below, before the same PCM is queued to SDL. */
extern "C" void beetle_host_audio_push(const int16_t *stereo, size_t frames);
