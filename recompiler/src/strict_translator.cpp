...
TranslateResult StrictTranslator::translate(const PSXRecomp::DecodedInstruction& d) {
    ...
    if (opcode == 0x00) {
        ...
        switch (funct) {
            ...
            default:
                r.supported = true;
                r.is_terminator = true;
                r.terminator_kind = "reserved_instruction";
                r.c_code = fmt::format(
                    "psx_reserved_instruction_exception(cpu, 0x{:08X}u); return;",
                    d.address);
                r.comment = fmt::format("reserved SPECIAL funct 0x{:02X}", funct);
                return r;
        }
    }
    ...
    // COP1 (0x11), LWC1 (0x31), SWC1 (0x39).
    //
    // The retail PS1 has no COP1/FPU. Beetle's PSX CPU model is the runtime
    // oracle here: when SR.CU1 (bit 29) is clear, every COP1-class instruction
    // raises Coprocessor Unusable (ExcCode=11, Cause.CE=1). If CU1 is set,
    // register/arithmetic operations are NOP-like; BC1F is always taken and
    // BC1T is never taken because there is no condition register. LWC1 performs
    // an aligned memory read and discards it; SWC1 only performs the alignment
    // check and does not write memory. This executes the instruction semantics;
    // it is not an HLE implementation of the BIOS strtod routine.
    if (opcode == 0x11) {
        const uint8_t cop_op = (d.raw >> 21) & 0x1F;
        const uint8_t rt_field = (d.raw >> 16) & 0x1F;

        if (cop_op == 0x08 || cop_op == 0x0C) { // BC1F / BC1T
            const int32_t simm = static_cast<int32_t>(static_cast<int16_t>(d.raw & 0xFFFF));
            const uint32_t target = d.address + 4 + static_cast<uint32_t>(simm * 4);
            const uint32_t fallthrough = d.address + 8;
            const bool taken = (rt_field & 1u) == 0u; // absent condition bit is false
            r.supported = true;
            r.is_terminator = true;
            r.terminator_kind = taken ? "branch_bc1f" : "branch_bc1t";
            r.terminator_target = target;
            r.pre_delay_code = fmt::format(
                "if (!(cpu->cop0[12] & (1u << 29))) {{ psx_cop_unusable(cpu, 1u, 0x{:08X}u); return; }} "
                "uint32_t psx_brA_{:08X} = {}u;",
                d.address, d.address, taken ? 1u : 0u);
            r.c_code = fmt::format(
                "if (!(cpu->cop0[12] & (1u << 29))) {{ psx_cop_unusable(cpu, 1u, 0x{:08X}u); return; }} "
                "if ({}u) {{ cpu->pc = 0x{:08X}u; return; }} cpu->pc = 0x{:08X}u; return;",
                d.address, taken ? 1u : 0u, target, fallthrough);
            r.comment = fmt::format("bc1{} 0x{:08X}", taken ? "f" : "t", target);
            return r;
        }
        ...
        r.c_code = fmt::format(
            "if (!(cpu->cop0[12] & (1u << 29))) {{ psx_cop_unusable(cpu, 1u, 0x{:08X}u); return; }} "
            "/* COP1 absent on PS1: enabled operation is NOP-like */",
            d.address);
        r.comment = fmt::format("cop1 rs=0x{:02X} (c1 unusable / nop)", cop_op);
        return r;
    }

    if (opcode == 0x31) { // LWC1 — absent coprocessor load
        const uint8_t rs = (d.raw >> 21) & 0x1F;
        const int32_t simm = static_cast<int32_t>(static_cast<int16_t>(d.raw & 0xFFFF));
        r.supported = true;
        r.c_code = fmt::format(
            "{{ uint32_t psx_addr = (uint32_t)((int32_t)cpu->gpr[{}] + ({})); "
            "if (!(cpu->cop0[12] & (1u << 29))) {{ psx_cop_unusable(cpu, 1u, 0x{:08X}u); return; }} "
            "if (psx_addr & 3u) {{ psx_address_error_exception(cpu, psx_addr, 0x{:08X}u, 0); return; }} "
            "(void)psx_cyc_cop_load_word(cpu, psx_addr); }}",
            static_cast<int>(rs), simm, d.address, d.address);
        r.comment = fmt::format("lwc1 (discard), {}({})", simm, gpr_name(rs));
        return r;
    }

    if (opcode == 0x39) { // SWC1 — absent coprocessor store
        const uint8_t rs = (d.raw >> 21) & 0x1F;
        const int32_t simm = static_cast<int32_t>(static_cast<int16_t>(d.raw & 0xFFFF));
        r.supported = true;
        r.c_code = fmt::format(
            "{{ uint32_t psx_addr = (uint32_t)((int32_t)cpu->gpr[{}] + ({})); "
            "if (!(cpu->cop0[12] & (1u << 29))) {{ psx_cop_unusable(cpu, 1u, 0x{:08X}u); return; }} "
            "if (psx_addr & 3u) {{ psx_address_error_exception(cpu, psx_addr, 0x{:08X}u, 1); return; }} "
            "/* SWC1 on absent enabled COP1 performs no memory write */ }}",
            static_cast<int>(rs), simm, d.address, d.address);
        r.comment = fmt::format("swc1 (no write), {}({})", simm, gpr_name(rs));
        return r;
    }
